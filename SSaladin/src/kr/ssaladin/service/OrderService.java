...
public class OrderService {
	...
	public OrderService() throws Exception {
		...
		} catch (SQLException e) {System.out.println("OrderService 초기화 중 오류 발생: " + e.getMessage());
			e.printStackTrace();
			throw e;
		}
	}
	...
	public boolean createOrder(String userId, int orderTotal, int orderStatus, List<OrderItem> orderItems) throws ClassNotFoundException {
		try {conn.setAutoCommit(false); // 트랜잭션 시작
			// 재고 확인
			if (!checkOrderItemsStock(orderItems)) { conn.rollback();
			return false;
			}
			...
			// 주문 상세 정보 생성 및 재고 업데이트
			if (!createOrderItemDetails(orderNum, orderItems)) {conn.rollback();
			return false;
			}
	...
	private int createOrderTransaction(String userId, int orderTotal, int orderStatus) throws SQLException {
		...
		if (!orderInserted) {System.out.println("주문 정보 등록에 실패했습니다.");
		return -1;
		}

		// 방금 생성한 주문번호 조회
		int orderNum = ordersDAO.getLastOrderNumByUser(userId);
		if (orderNum == -1) {System.out.println("주문번호 조회에 실패했습니다.");
		return -1;
		}	return orderNum;
	}
	...
	private boolean createOrderDetails(int orderNum, List<CartItem> items) throws SQLException, ClassNotFoundException {
		for (CartItem item : items) {
			...
			if (!detailInserted) {System.out.println("주문 상세 정보 등록에 실패했습니다.");
			return false;
			}
	...
	private boolean updateBookStock(int bookCode, int quantity) throws SQLException, ClassNotFoundException {
		...
		if (!stockUpdated) {System.out.println("재고 차감에 실패했습니다.");
		return false;
		}

		// 재고가 0일 경우 품절 상태로 변경
		boolean statusUpdated = adminBookDAO.updateBookStatus(bookCode);
		if (!statusUpdated) {System.out.println("도서 상태 업데이트에 실패했습니다.");
		return false;
		}
	...
	public boolean cancelOrder(int orderNum) {
		try {
			...
			if (orderInfo == null) {System.out.println("해당 주문 정보가 없습니다.");
			return false;
			}
			// 취소 가능 상태 확인
			if (!isOrderCancellable(orderInfo)) {return false;}
			// 주문 상태를 '취소'로 변경
			if (!ordersDAO.updateOrderStatus(orderNum, OrderStatus.CANCELLED)) {System.out.println("주문 상태 변경에 실패했습니다.");
			conn.rollback();
			return false;
			}
			//도서 재고 복구
			if (!restoreOrderStock(orderInfo.getOrderDetails())) {conn.rollback();
			return false;
			}
			// 환불 전 포인트 조회
...
/*
	...
	*/
	...
	public boolean processOrder(int orderNum, int newStatus) {
		try {
			...
			// 이미 취소된 주문은 상태 변경 불가
			if (orderInfo.getOrderStatus() == OrderStatus.CANCELLED) {System.out.println("취소된 주문의 상태는 변경할 수 없습니다.");
				return false;
			}
			return ordersDAO.updateOrderStatus(orderNum, newStatus);
		} catch (SQLException e) {System.out.println("주문 처리 중 오류 발생: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
	}
	...
	public List<OrderInfo> getUserOrders(String userId) {
		...
		} catch (SQLException e) {System.out.println("사용자 주문 목록 조회 중 오류 발생: " + e.getMessage());
			e.printStackTrace();
			return null;
		}
		return orderList;
	...
	public List<OrderInfo> getAllOrders() {
		...
		} catch (SQLException e) {System.out.println("전체 주문 목록 조회 중 오류 발생: " + e.getMessage());
			e.printStackTrace();
			return null;
		}
		return orderList;
	...
	*/
	...
	private List<OrderDetailInfo> getOrderDetailsByOrderNum(int orderNum) {
		...
		} catch (SQLException e) {System.out.println("주문 상세 정보 조회 중 오류 발생: " + e.getMessage());
			e.printStackTrace();
		}
		return details;
	}
	...
	public void displayUserOrderHistory(String userId) {
		try {
			...
			for (OrderInfo order : orders) {
				...
				} else {System.out.println("\t상세 정보가 없습니다.");
				}System.out.println("-".repeat(45));
			}System.out.println("=".repeat(45));
		} catch (Exception e) {System.out.println("주문 내역 조회 중 오류 발생: " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public void displayAllOrders() {
		...
		} catch (Exception e) {System.out.println("전체 주문 내역 조회 중 오류 발생: " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public void checkOrderStatus(String userId, int userAuth) throws IOException {
		...
		if (userOrders == null || userOrders.isEmpty()) {System.out.println("주문 내역이 없습니다.");
			return;
		}
		// 주문 목록 출력
	...
	// 주문 목록 표시
	private void displayOrderList(List<OrderInfo> orders) {System.out.println("주문번호\t주문일자\t\t주문상태\t주문금액");
	...
	// 주문 상세 조회 및 취소 처리
	private void handleOrderActions(String userId, int userAuth) throws IOException {System.out.print("\n상세 조회할 주문 번호를 입력하세요 (돌아가려면 0 입력): ");
		try {
			int orderNum = Integer.parseInt(br.readLine().trim());

			if (orderNum == 0) {return; // 메인 메뉴로 돌아가기
			}
			...
			if (orderInfo != null) {
				// 접근 권한 확인
				if (userAuth != 2 && !orderInfo.getUserId().equals(userId)) {System.out.println("해당 주문에 대한 접근 권한이 없습니다.");
					return;
				}
				displayOrderDetails(orderInfo);
				
				// 결제 완료 상태인 경우 취소 옵션 제공
				if (orderInfo.getOrderStatus() == OrderStatus.PROCESSING) {offerCancelOption(orderNum);}
			} else {System.out.println("해당 주문 정보를 찾을 수 없습니다.");}
		} catch (NumberFormatException e) {System.out.println("올바른 주문 번호를 입력해주세요.");
		} catch (Exception e) {System.out.println("주문 조회 중 오류가 발생했습니다: " + e.getMessage());
			e.printStackTrace();
		}
	}

	// 주문 취소 옵션 제공
	private void offerCancelOption(int orderNum) throws IOException {System.out.print("\n이 주문을 취소하시겠습니까? (Y/N): ");
		String choice = br.readLine().trim().toUpperCase();

		if (choice.equals("Y")) {
			if (cancelOrder(orderNum)) {System.out.println("주문이 성공적으로 취소되었습니다.");
			}
		}
	}
	...
	public void manageUserOrders(String userId) throws IOException {
		...
		while (running) {
			...
			try {
				...
				switch (choice) {
				case 1:displayUserOrderHistory(userId);
					break;
				case 2:promptAndShowOrderDetails(userId, false);
					break;
				case 3:promptAndCancelOrder(userId);
					break;
	...
	private void promptAndShowOrderDetails(String userId, boolean isAdmin) throws IOException {
		...
		try {
			...
			if (orderInfo != null && (isAdmin || orderInfo.getUserId().equals(userId))) {
				displayOrderDetails(orderInfo);
			} else {System.out.println("해당 주문 정보가 없거나 접근 권한이 없습니다.");
			}
		} catch (NumberFormatException e) {System.out.println("유효한 주문번호를 입력해주세요.");
		}
	}
		...
		while (running) {
			...
			try {
				...
				case 1:displayAllOrders();
					break;
				case 2:promptAndShowOrderDetails(null, true);
					break;
				case 3:promptAndChangeOrderStatus();
					break;
				case 4:promptAndCancelOrderAdmin();
					break;
		...
		try {
			...
			if (orderInfo != null) {
				...
				if (newStatus >= 1 && newStatus <= 5) {
					if (processOrder(orderNum, newStatus)) {System.out.println("주문 상태가 변경되었습니다.");
					}
				} else {System.out.println("잘못된 상태 값입니다.");
				}
			} else {System.out.println("해당 주문 정보가 없습니다.");
			}
		} catch (NumberFormatException e) {System.out.println("유효한 숫자를 입력해주세요.");
		}
	}
	...
	private void promptAndCancelOrderAdmin() throws IOException {
		...
		try {
			...
			if (orderInfo != null) {System.out.print("정말 취소하시겠습니까? (Y/N): ");
			String confirm = br.readLine().trim().toUpperCase();

			if (confirm.equals("Y")) {
				if (cancelOrder(orderNum)) {System.out.println("주문이 취소되었습니다.");
				}
			}
			} else {System.out.println("해당 주문 정보가 없습니다.");
			}
