...
public class SSaladinMain {
  ...
  //장바구니 관리
	   boolean cartMenu = true;
	      while (cartMenu) {
	         System.out.println("\n=== 장바구니 관리 ===");
	         System.out.print("1. 장바구니 목록 보기, 2. 장바구니 상품 수량 변경, "
	               + "3. 장바구니 상품 삭제, 4. 뒤로가기: ");
	         try {
	            int no = Integer.parseInt(br.readLine());
	            if (no == 1) {
...
/*
   private void showCartItems() throws IOException {
	    ...
	    try {
	        ...
	        // 로그인된 사용자 ID(me_id)로 장바구니 조회
	        cartService.getUserCartItems(me_id ).forEach(item -> 
	            System.out.println("도서코드: " + item.getProductId() + 
	                               ", 도서명: " + item.getProductName() + 
	                               ", 수량: " + item.getQuantity() + 
	                               ", 가격: " + item.getPrice()));
	    } catch (Exception e) {
