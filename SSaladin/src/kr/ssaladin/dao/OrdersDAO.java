...
public class OrdersDAO {
	...
	// 주문번호로 주문 조회 후 Orderinfo 객체 반환
	public OrderInfo getOrderInfo(int orderNum) throws SQLException {
		...
		try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				if (rs.next()) {
					...
					orderInfo.setOrderNum(rs.getInt("order_num"));
					orderInfo.setUserId(rs.getString("user_id"));
					orderInfo.setOrderTotal(rs.getInt("order_total"));
	...
	// 사용자의 현재 포인트 조회
	public int getUserCurrentPoint(String userId) throws SQLException {
		...
		// 재고 복구 메서드
		public boolean restoreStock(int bookCode, int quantity) throws SQLException {
		...
		// 도서 상태 업데이트 (품절 -> 판매중)
		public boolean updateBookStatusAfterCancel(int bookCode) throws SQLException {
